...
// XXX forkbomb: many of these changes are unused? try reverting see what happens.
...
#ifdef CONFIG_MACH_LAVENDER
	if (!strcmp(slave_info->eeprom_name, "lavender_s5k5e8_ofilm_i") ||
			!strcmp(slave_info->eeprom_name, "lavender_s5k5e8_sunny_ii") ||
			!strcmp(slave_info->eeprom_name, "lavender_ov02a10_ofilm_i") ||
			!strcmp(slave_info->eeprom_name, "lavender_ov02a10_sunny_ii")) {

		for (i=0; i<CAMERA_VENDOR_EEPROM_COUNT_MAX; i++) {
			if (s_vendor_eeprom[i].eeprom_name != NULL) {
				pr_debug(" slave_info->eeprom_name=%s, s_vendor_eeprom[%d]=%s, module_id=%d,\n",
					slave_info->eeprom_name, i, s_vendor_eeprom[i].eeprom_name, s_vendor_eeprom[i].module_id);
				if (!strcmp(slave_info->eeprom_name, s_vendor_eeprom[i].eeprom_name)) {
					if ((!strcmp(slave_info->eeprom_name, "lavender_s5k5e8_ofilm_i") && (s_vendor_eeprom[i].module_id == MID_OFILM))
							|| (!strcmp(slave_info->eeprom_name, "lavender_s5k5e8_sunny_ii") && (s_vendor_eeprom[i].module_id == MID_SUNNY))
							|| (!strcmp(slave_info->eeprom_name, "lavender_ov02a10_ofilm_i") && (s_vendor_eeprom[i].module_id == MID_OFILM))
							|| (!strcmp(slave_info->eeprom_name, "lavender_ov02a10_sunny_ii") && (s_vendor_eeprom[i].module_id == MID_SUNNY))) {
						break;
					}
				}
			}
...
#ifdef CONFIG_MACH_XIAOMI
			(slave_info->vendor_id_info.vendor_id == s_ctrl->sensordata->cam_slave_info->vendor_id_info.vendor_id) &&
#endif
