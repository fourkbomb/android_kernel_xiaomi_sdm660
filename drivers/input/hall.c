...
static int hall_parse_dt(struct device *dev, struct hall_switch_info *pdata)
{
	...
	dev_debug(dev, "Macle hall irq_gpio=%d\n", pdata->irq_gpio);
	return 0;
}
...
static int hall_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "Macle hall_probe\n");

	if (pdev->dev.of_node) {
		hall_info = kzalloc(sizeof(struct hall_switch_info), GFP_KERNEL);
		if (!hall_info) {
			dev_err(&pdev->dev, "Macle %s:hall failed to alloc memory for module data\n", __func__);
			return -ENOMEM;
		}
	...
	if (!hall_info->ipdev) {
		dev_err(&pdev->dev, "hall_probe: input_allocate_device fail\n");
		goto input_error;
	}
	...
	if (rc) {
		dev_err(&pdev->dev, "hall_probe: input_register_device fail rc=%d\n", rc);
		goto input_error;
	}
	...
	if (gpio_is_valid(hall_info->irq_gpio)) {
		...
		if (rc < 0) {
			dev_err(&pdev->dev, "hall_probe: request_irq fail rc=%d\n", rc);
			goto err_irq;
		}
		...
		dev_err(&pdev->dev, "Macle hall irq gpio not provided\n");
		goto free_input_device;
	}

	dev_debug(&pdev->dev, "hall_probe end\n");
#ifdef CONFIG_HALL_SYS
...
#endif
	dev_debug(&pdev->dev, "hall_remove\n");
