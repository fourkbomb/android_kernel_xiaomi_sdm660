...
static int ant_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "Macle ant_probe\n");

	if (pdev->dev.of_node) {
		ant_info = kzalloc(sizeof(struct ant_check_info), GFP_KERNEL);
		if (!ant_info) {
			dev_err(&pdev->dev, "Macle %s: failed to alloc memory for module data\n", __func__);
			return -ENOMEM;
		}
	...
	if (!ant_info->ipdev) {
		dev_dbg(&pdev->dev, "ant_probe: input_allocate_device fail\n");
		goto input_error;
	}
	...
	if (rc) {
		dev_err(&pdev->dev, "ant_probe: input_register_device fail rc=%d\n", rc);
		goto input_error;
	}
	...
	if (gpio_is_valid(ant_info->irq_gpio)) {
		...
		if (rc < 0) {
			dev_err(&pdev->dev, "ant_probe: request_irq fail rc=%d\n", rc);
			goto err_irq;
		}
#if defined(CONFIG_ANT_CHECK_D2S_CN)
		...
		dev_err(&pdev->dev, "Macle irq gpio not provided\n");
		goto free_input_device;
	}
	dev_dbg(&pdev->dev, "ant_probe end\n");
#ifdef CONFIG_ANT_SYS
